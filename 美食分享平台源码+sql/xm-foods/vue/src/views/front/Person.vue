<template>
  <div class="main-content">
    ...
    <el-dialog title="充值" :visible.sync="chargeVisible" width="30%" :close-on-click-modal="false" destroy-on-close>
      <el-form label-width="120px" style="padding-right: 20px">
        <div style="text-align: center; margin-bottom: 10px">
          <img style="width: 70%" src="@/assets/imgs/pay.png" alt="">
        </div>
        <el-form-item label="充值金额" prop="account">
          <el-input-number :min="1" v-model="account" placeholder="金额"></el-input-number>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button @click="chargeVisible = false">取 消</el-button>
        <el-button type="primary" @click="saveCharge">确认支付</el-button>
      </div>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    saveCharge() {
      this.$request.put('/user/charge/' + this.user.id + '/' + this.account).then(res => {
        if (res.code === '200') {
